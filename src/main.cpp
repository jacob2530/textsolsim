...
int main() {
    ...
    while (!exitProgram) {
        ...
        mvprintw(screen_rows - 10, screen_cols / 2 - 13, "Press ESC to Quit");

        int choice_y = screen_rows / 2 + 1;
        int choice_x = screen_cols / 2 - 13 + 8;
        move(choice_y, choice_x);
        refresh();
        ...
        // Clear the stars and planets vectors
        stars.clear();
        planets.clear();

        // Return to the main menu when 'q' is pressed
        clear();
        goto main_menu;
    }
    endwin();
